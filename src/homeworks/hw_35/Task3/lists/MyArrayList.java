package homeworks.hw_35.Task3.lists;
...
public class MyArrayList<T> implements MyList<T> {
    ...
    public T remove(int index) {
        ...
        T value = array[index]; // Запоминаем удаляемое значение
 ...
 */
    ...
    public void removeDuplicates(T target) {
        for (int i = 0; i < size(); ) {
            if (get(i).equals(target)) {
                remove(i); // Удаляем, но не увеличиваем i
            } else {
                i++; // Только если элемент не удален, двигаемся дальше
            }
        }
    }
